...
// ============================================================
// LOCAL PAYWALL UNLOCK
// Single switch: report the top tier for all *client-side* gates so the
// extension behaves as fully unlocked locally. Server-backed features
// (Insights / Alerts) still verify the license against the backend, so those
// remain dependent on a valid key + a reachable backend.
// Set UNLOCK_TIER back to `null` to restore normal license-driven behavior.
// ============================================================
const UNLOCK_TIER = TIERS.PRO_WEB_ALERTS;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "GET_LICENSE") {
    chrome.storage.local.get("license", (data) => {
      if (UNLOCK_TIER != null) {
        // Report top tier to the popup so all UI unlocks. Preserve a real key
        // if one was activated; otherwise supply a label so the masked-key
        // display has something to render.
        const stored = data.license || {};
        sendResponse({
          license: { ...stored, level: UNLOCK_TIER, key: stored.key || "LOCAL-UNLOCK" },
        });
      } else {
...
async function enforceGameLimit(gameKey) {
  const data = await getStorage();
  const level = effectiveLevel(data.license?.level);
